...
!function (window) {
    ...
    lay.isTopElem = function (elem) {
        var topElems = [document, lay("body")[0]], matched = false;
    ...
    lay.clipboard = {
        writeText: function (options) {
            ...
            try {
                navigator.clipboard.writeText(text).then(options.done)["catch"](options.error)
            } catch (e) {
...
!function (window, document) {
    var isLayui = window.layui && layui.define, ready = {
        getPath: (window.lay && lay.getPath) ? lay.getPath : "", link: function (href, fn, cssname) {
            if (!laydate.path) {
                return
            }
    ...
    var laydate = {
        ...
        ready: function (callback) {
            ...
            isLayui ? (layui["layui.all"] ? (typeof callback === "function" && callback()) : layui.addcss(path, callback, cssname)) : ready.link(path, callback, cssname);
            return this
        }
    };
    ...
    var Class = function (options) {
        ...
        if (elem.length > 1) {
            lay.each(elem, function () {
                laydate.render(lay.extend({}, that.config, {elem: this}))
            });
            return that
        }
        options = lay.extend(that.config, lay.options(elem[0]));
    ...
    Class.prototype.init = function () {
        var that = this, options = that.config, isStatic = options.position === "static", format = {
            year: "yyyy",
            month: "yyyy-MM",
            date: "yyyy-MM-dd",
            time: "HH:mm:ss",
        ...
        options.elem.attr(MOD_ID, options.id);
        ...
        lay.each(["min", "max"], function (i, item) {
            ...
            if (typeof options[item] === "number") {
                var day = options[item], tDate = new Date(), time = that.newDate({
    ...
    Class.prototype.render = function () {
        var that = this, options = that.config, lang = that.lang(), isStatic = options.position === "static",
            elem = that.elem = lay.elem("div", {
                id: that.elemID,
                "class": ["layui-laydate", options.range ? " layui-laydate-range" : "", that.rangeLinked ? " layui-laydate-linkage" : "", isStatic ? (" " + ELEM_STATIC) : "", options.fullPanel ? " laydate-theme-fullpanel" : "", (function () {
                    var themeStr = "";
            ...
            elemTable = that.table = [], divFooter = that.footer = lay.elem("div", {"class": ELEM_FOOTER}),
            divShortcut = that.shortcut = lay.elem("ul", {"class": ELEM_SHORTCUT});
        if (options.zIndex) {
        ...
        lay.each(new Array(2), function (i) {
            ...
            elemMain[i] = lay.elem("div", {"class": ELEM_MAIN + " laydate-main-list-" + i});
        ...
        typeof options.ready === "function" && options.ready(lay.extend({}, options.dateTime, {month: options.dateTime.month + 1}));
        that.preview()
    };
    ...
    Class.prototype.position = function () {
        ...
        lay.position(options.elem[0], that.elem, {position: options.position});
        return that
    };
    ...
    Class.prototype.checkDate = function (fn) {
        var that = this, thisDate = new Date(), options = that.config, lang = that.lang(),
            dateTime = options.dateTime = options.dateTime || that.systemDate(), thisMaxDate, error,
            elem = options.elem[0], valType = that.isInput(elem) ? "val" : "html", value = function () {
                if (that.rangeElem) {
        ...
        if (fn === "limit") {
            if (options.range) {
                checkValid(that.rangeLinked ? that.startDate : dateTime);
                that.endDate && checkValid(that.endDate)
            } else {
                checkValid(dateTime)
            }
            return that
        ...
        var getEndDate = function () {
            if (options.range) {
                that.endDate = that.endDate || lay.extend({}, options.dateTime, function () {
                    var obj = {}, dateTime = options.dateTime, EYM = that.getAsYM(dateTime.year, dateTime.month);
        ...
        if (options.range) {
            ...
            that.endTime = {hours: that.endDate.hours, minutes: that.endDate.minutes, seconds: that.endDate.seconds,};
        ...
        that.startDate = that.startDate || value && lay.extend({}, options.dateTime);
        that.autoCalendarModel.auto && that.autoCalendarModel();
        that.endState = !options.range || !that.rangeLinked || !!(that.startDate && that.endDate);
        fn && fn();
        return that
    };
        ...
        lay.each(options.holidays, function (idx, item) {
            lay.each(item, function (i, dayStr) {
                if (dayStr === td.attr("lay-ymd")) {
                    td.find("div").html('<span class="laydate-day-holidays"' + (type[idx] ? ('type="' + type[idx] + '"') : "") + ">" + YMD[2] + "</span>")
                }
            })
        });
        return that
    ...
    Class.prototype.limit = function (opts) {
        ...
        lay.each({
            ...
            timestamp[key] = that.newDate(lay.extend({
                ...
                date: (opts.type === "year" || opts.type === "month") ? 1 : item.date
            }, function () {
                var hms = {};
    ...
    Class.prototype.calendar = function (value, index, type) {
        index = index ? 1 : 0;
        var that = this, options = that.config, dateTime = value || that.thisDateTime(index), thisDate = new Date(),
            startWeek, prevMaxDate, thisMaxDate, lang = that.lang(),
            isAlone = options.type !== "date" && options.type !== "datetime", tds = lay(that.table[index]).find("td"),
        ...
        lay.each(tds, function (index_, item) {
            ...
            that.mark(item, YMD).holidays(item, YMD).limit({
                elem: item,
                date: {year: YMD[0], month: YMD[1] - 1, date: YMD[2]},
                index: index_
            })
        });
        ...
        if (options.range && type === "init") {
            if (that.rangeLinked) {
                ...
                that.calendar(lay.extend({}, dateTime, {year: EYM[0], month: EYM[1]}), 1 - index)
            } else {
        ...
        if (!options.range) {
            ...
            that.limit({elem: lay(that.footer).find(ELEM_CONFIRM), index: 0, time: timeParams})
        }
        that.setBtnStatus();
    ...
    Class.prototype.list = function (type, index) {
        var that = this, options = that.config,
            ...
            lang = that.lang(), isAlone = options.range && options.type !== "date" && options.type !== "datetime",
            ul = lay.elem("ul", {
                "class": ELEM_LIST + " " + ({
            ...
            isCN = options.lang === "cn", text = isCN ? "年" : "", listYM = that.listYM[index] || {},
        ...
        if (type === "year") {
            ...
            lay.each(new Array(15), function (i) {
                var li = lay.elem("li", {"lay-ym": yearNum}), ymd = {year: yearNum, month: 0, date: 1};
                yearNum == listYM[0] && lay(li).addClass(THIS);
                li.innerHTML = yearNum + text;
                ul.appendChild(li);
                that.limit({elem: lay(li), date: ymd, index: index, type: type});
                yearNum++
            });
            ...
            if (type === "month") {
                lay.each(new Array(12), function (i) {
                    var li = lay.elem("li", {"lay-ym": i}), ymd = {year: listYM[0], month: i, date: 1};
                            ...
                            lay(ol).find("li").each(function (ii, li) {
                                that.limit({
                                    ...
                                    date: [{hours: ii}, {
                                        ...
                                        minutes: ii
                                    }, {hours: that[startEnd].hours, minutes: that[startEnd].minutes, seconds: ii}][i],
                                    index: index,
        ...
        if (type === "year" || type === "month") {
            ...
            lay(ul).find("li").on("click", function () {
                ...
                if (isYearOrMonth) {
                    ...
                    that.checkDate("limit").calendar(dateTime, index, "init");
                    that.closeList()
                }
                that.setBtnStatus();
            ...
            var span = lay.elem("span", {"class": ELEM_TIME_TEXT}), scroll = function () {
    ...
    Class.prototype.setBtnStatus = function (tips, start, end) {
        var that = this, options = that.config, lang = that.lang(), isOut,
        ...
        if (options.range && options.type !== "time") {
            ...
            (that.limit({date: start}) || that.limit({date: end})) ? elemBtn.addClass(DISABLED) : elemBtn[isOut ? "addClass" : "removeClass"](DISABLED);
            if (tips && isOut) {
    ...
    Class.prototype.setValue = function (value) {
        var that = this, options = that.config, elem = options.elem[0];
        if (options.position === "static") {
            return that
        }
        value = value || "";
    ...
    Class.prototype.preview = function () {
        ...
        oldValue && (elemPreview.css({"color": "#16b777"}), setTimeout(function () {
            elemPreview.css({"color": "#777"})
        }, 300))
    };
    ...
    Class.prototype.stampRange = function () {
        var that = this, options = that.config, startTime = that.rangeLinked ? that.startDate : options.dateTime,
            endTime, tds = lay(that.elem).find("td");
        ...
        lay.each(tds, function (i, item) {
            ...
            var thisTime = that.newDate({year: ymd[0], month: ymd[1] - 1, date: ymd[2]}).getTime();
    ...
    Class.prototype.done = function (param, type) {
        ...
        lay.each([start, end], function (i, item) {
            ...
            lay.extend(item, {month: item.month + 1})
        });
        that.preview();
    ...
    Class.prototype.choose = function (td, index) {
        ...
        var that = this, options = that.config, panelIndex = index;
        if (that.rangeLinked) {
        ...
        var dateTime = that.thisDateTime(index), tds = lay(that.elem).find("td"), YMD = td.attr("lay-ymd").split("-");
        YMD = {year: YMD[0] | 0, month: (YMD[1] | 0) - 1, date: YMD[2] | 0};
        lay.extend(dateTime, YMD);
        if (options.range) {
            lay.each(["startTime", "endTime"], function (i, item) {
                that[item] = that[item] || {hours: i ? 23 : 0, minutes: i ? 59 : 0, seconds: i ? 59 : 0};
                if (index === i) {
            ...
            if (that.endState && !that.limit({date: that.thisDateTime(1 - index)})) {
                ...
                if ((isChange || that.rangeLinked && that.endState) && that.newDate(that.startDate) > that.newDate(that.endDate)) {
                    ...
                    isSameDate && (startDate = that.startTime, that.startTime = that.endTime, that.endTime = startDate)
                }
                isChange && (options.dateTime = lay.extend({}, that.startDate))
            }
            if (that.rangeLinked) {
                var dateTimeTemp = lay.extend({}, dateTime);
                if (panelIndex && !index && !isChange) {
                    var YM = that.getAsYM(dateTime.year, dateTime.month, "sub");
                    lay.extend(options.dateTime, {year: YM[0], month: YM[1]})
                }
    ...
    Class.prototype.tool = function (btn, type) {
        var that = this, options = that.config, lang = that.lang(), dateTime = options.dateTime,
            isStatic = options.position === "static", active = {
                datetime: function () {
    ...
    Class.prototype.change = function (index) {
        var that = this, options = that.config, dateTime = that.thisDateTime(index),
            isAlone = options.range && (options.type === "year" || options.type === "month"),
            elemCont = that.elemCont[index || 0], listYM = that.listYM[index], addSubYear = function (type) {
                ...
                if (isYear || isMonth) {
                    ...
                    options.range || that.limit({elem: lay(that.footer).find(ELEM_CONFIRM), date: {year: listYM[0]}})
                }
                that.setBtnStatus();
    ...
    ready.run = function (lay) {
        lay(document).on("mousedown", function (e) {
            ...
            var options = that.config;
            if (e.target === options.elem[0] || e.target === options.eventElem[0] || e.target === lay(options.closeStop)[0] || (options.elem[0] && options.elem[0].contains(e.target))) {
                return
            }
            that.remove()
    ...
    isLayui ? (laydate.ready(), layui.define("lay", function (exports) {
