...
$.fn.addtabs = function (options) {
    ...
    _close = function (id) {
        ...
        if (obj.find("li.active").attr('id') == "tab_" + id) {
            if ($("#tab_" + id).prev().not(".tabdrop").length) > 0) {
                $("#tab_" + id).prev().not(".tabdrop").find("a").trigger("click");
            } else if ($("#tab_" + id).next().length) > 0) {
                        ...
                        if ($(this).hasClass(d.css_class.container_open)) {
                            ...
                            if ($('li', $(d.elem.results)).length) > 0) {
                                if (d.option.auto_fill_result) {//打开自动内容填充功能
                                    //若已有选中项目，则直接隐藏列表
                                    if ($('li.sp_selected', $(d.elem.results)).length) > 0) {
                                        d._hideResults(d);
                                    } else if ($('li.sp_over', $(d.elem.results)).length) > 0) {
                                        //若控件已有选中的值，则忽略高亮的项目
