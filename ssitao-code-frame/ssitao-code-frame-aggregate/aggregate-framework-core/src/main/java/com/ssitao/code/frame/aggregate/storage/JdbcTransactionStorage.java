...
import com.ssitao.code.frame.aggregate.storage.domain.AlertType;
import com.ssitao.code.frame.aggregate.storage.domain.DomainStore;
...
public class JdbcTransactionStorage extends AbstractTransactionStorage implements StorageRecoverable {
    ...
    private static final String SQL_SELECT_PREFIX_FOR_AGGREGATE_FRAMEWORK = "SELECT DOMAIN,XID,CONTENT,CREATE_TIME,LAST_UPDATE_TIME,RETRIED_COUNT,VERSION,IS_DELETE,REQUEST_ID FROM ";
    private static final String SQL_SELECT_PREFIX_FOR_AGG_DOMAIN = "SELECT DOMAIN, MAX_RETRY_COUNT, PHONE_NUMBERS, ALERT_TYPE, THRESHOLD, INTERVAL_MINUTES,LAST_ALERT_TIME,DING_ROBOT_URL,CREATE_TIME,LAST_UPDATE_TIME,VERSION FROM ";
    ...
    protected int doCreate(TransactionStore transactionStore) {
        ...
        try {
            ...
            builder
                    .append("INSERT INTO ")
                    .append(getTableName())
                    .append("(XID,CONTENT,RETRIED_COUNT,CREATE_TIME,LAST_UPDATE_TIME,VERSION,DOMAIN,REQUEST_ID) VALUES (?,?,?,?,?,?,?,?)");

            stmt = connection.prepareStatement(builder.toString());

            stmt.setString(1, transactionStore.getXid().toString());
            stmt.setBytes(2, transactionStore.getContent());
            stmt.setInt(3, transactionStore.getRetriedCount());
            stmt.setTimestamp(4, new Timestamp(transactionStore.getCreateTime().getTime()));
            stmt.setTimestamp(5, new Timestamp(transactionStore.getLastUpdateTime().getTime()));
            stmt.setLong(6, transactionStore.getVersion());
            stmt.setString(7, transactionStore.getDomain());
            if (transactionStore.getRequestId() != null) {
                stmt.setInt(8, transactionStore.getRequestId());
            } else {
    ...
    protected int doUpdate(TransactionStore transactionStore) {
        ...
        try {
            ...
            builder.append("UPDATE " + getTableName() + " SET " +
                    "REQUEST_ID = ?,LAST_UPDATE_TIME = ?, RETRIED_COUNT = ?,VERSION = VERSION+1 WHERE XID = ? AND VERSION = ?");
            ...
            if (transactionStore.getRequestId() != null) {
                stmt.setInt(1, transactionStore.getRequestId());
            } else {
            ...
            stmt.setInt(3, transactionStore.getRetriedCount());
            stmt.setString(4, transactionStore.getXid().toString());
            stmt.setLong(5, transactionStore.getVersion() - 1);
    ...
    public void registerDomain(DomainStore domainStore) {
        ...
        executeUpdate(connection -> {
            ...
            builder.append("INSERT INTO " + getDomainTableName() +
                    "(DOMAIN,MAX_RETRY_COUNT,PHONE_NUMBERS,ALERT_TYPE,THRESHOLD,INTERVAL_MINUTES,LAST_ALERT_TIME,DING_ROBOT_URL,CREATE_TIME,LAST_UPDATE_TIME,VERSION)");
            builder.append(" VALUES (?,?,?,?,?,?,?,?,?,?,?)");

            PreparedStatement stmt = connection.prepareStatement(builder.toString());
            stmt.setString(1, domainStore.getDomain());
            stmt.setInt(2, domainStore.getMaxRetryCount());
            stmt.setString(3, domainStore.getPhoneNumbers());
            stmt.setString(4, domainStore.getAlertType() == null ? null : domainStore.getAlertType().name());
            ...
            stmt.setTimestamp(9, new Timestamp(domainStore.getCreateTime().getTime()));
    ...
    public void updateDomain(DomainStore domainStore) {
        executeUpdate(connection -> {
            ...
            stmt.setString(3, domainStore.getAlertType().name());
            ...
            stmt.setTimestamp(8, new Timestamp(domainStore.getLastUpdateTime().getTime()));
            stmt.setString(9, domainStore.getDomain());
    ...
    public DomainStore findDomain(String domain) {
        ...
        List<DomainStore> list = executeDomainQuery(connection -> {
            StringBuilder builder = new StringBuilder();
            builder.append(SQL_SELECT_PREFIX_FOR_AGG_DOMAIN + getDomainTableName() + " WHERE DOMAIN = ?");
    ...
    public List<DomainStore> getAllDomains() {
        return executeDomainQuery(connection -> {
            StringBuilder builder = new StringBuilder();
            builder.append(SQL_SELECT_PREFIX_FOR_AGG_DOMAIN + getDomainTableName());
            PreparedStatement stmt = connection.prepareStatement(builder.toString());
            return stmt;
        });
    }
    ...
    private Page<TransactionStore> pageList(String domain, Date date, String offset, int pageSize, boolean isMarkDeleted) {
        ...
        try {
            ...
            StringBuilder builder = new StringBuilder();
            builder.append(SQL_SELECT_PREFIX_FOR_AGGREGATE_FRAMEWORK + getTableName() + " WHERE LAST_UPDATE_TIME < ?");
    ...
    private List<TransactionStore> doFinds(String domain, List<Xid> xids, boolean isMarkDeleted) {
        ...
        try {
            ...
            StringBuilder builder = new StringBuilder();
            builder.append(SQL_SELECT_PREFIX_FOR_AGGREGATE_FRAMEWORK + getTableName() + " WHERE");
    ...
    private void constructTransactions(ResultSet resultSet, List<TransactionStore> transactions) throws SQLException {
        while (resultSet.next()) {
            ...
            transactionStore.setXid(new TransactionXid(resultSet.getString(2)));
            transactionStore.setContent(resultSet.getBytes(3));
            transactionStore.setCreateTime(resultSet.getTimestamp(4));
            transactionStore.setLastUpdateTime(resultSet.getTimestamp(5));
            transactionStore.setRetriedCount(resultSet.getInt(6));
            transactionStore.setVersion(resultSet.getLong(7));
    ...
    private List<DomainStore> constructDomains(ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            domainStore.setPhoneNumbers(resultSet.getString(3));
            domainStore.setAlertType(AlertType.nameOf(resultSet.getString(4)));
            domainStore.setThreshold(resultSet.getInt(5));
            domainStore.setIntervalMinutes(resultSet.getInt(6));
            domainStore.setLastAlertTime(resultSet.getDate(7));
            domainStore.setDingRobotUrl(resultSet.getString(8));
            domainStore.setCreateTime(resultSet.getTimestamp(9));
            domainStore.setLastUpdateTime(resultSet.getTimestamp(10));
            domainStore.setVersion(resultSet.getLong(11));
    ...
    private String getTableName() {
        return StringUtils.isNotEmpty(tbSuffix) ? "AGGREGATE_FRAMEWORK_" + tbSuffix : "AGGREGATE_FRAMEWORK";
    }

    private String getDomainTableName() {
        return StringUtils.isNotEmpty(tbSuffix) ? "AGG_DOMAIN_" + tbSuffix : "AGG_DOMAIN";
    }
