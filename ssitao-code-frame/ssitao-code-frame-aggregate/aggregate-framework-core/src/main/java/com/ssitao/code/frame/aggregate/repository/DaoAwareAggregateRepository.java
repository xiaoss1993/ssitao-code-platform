package com.ssitao.code.frame.aggregate.repository;

import com.ssitao.code.frame.aggregate.dao.AggregateDao;
...
import com.ssitao.code.frame.aggregate.entity.DomainObject;
...
import com.ssitao.code.frame.aggregate.utils.DomainObjectUtils;
...
//import com.ssitao.code.frame.aggregate.utils.AnnotationUtils;
...
public class DaoAwareAggregateRepository<T extends AggregateRoot<ID>, ID extends Serializable> extends TraversalAggregateRepository<T, ID> {
    ...
    protected long doCount() {
        AggregateRootDao<T, ID> dao = (AggregateRootDao<T, ID>) AggreateDaoFactory.getDao(this.aggregateType);
        return dao.count();
    }
    ...
    protected T doFindOneDomainObject(Class<T> entityClass, ID id) {
        DomainObjectDao<T, ID> dao = AggreateDaoFactory.getDao(entityClass);
    ...
    protected List<T> doFindAllDomainObjects(Class<T> aggregateType) {
        AggregateRootDao<T, ID> dao = (AggregateRootDao<T, ID>) AggreateDaoFactory.getDao(aggregateType);
    ...
    protected <E extends DomainObject<I>, I extends Serializable> List<E> doFindAllDomainObjects(Class<E> entityClass, List<I> ids) {
        DomainObjectDao<E, I> dao = AggreateDaoFactory.getDao(entityClass);
    ...
    protected <I extends Serializable, E extends DomainObject<I>> Map<I, List<DomainObject<Serializable>>> doFindAllOneToManyDomainObjects(Class<E> entityClass, Field oneToManyField, Collection<I> ids) {
        ...
        DomainObjectDao oneToManyComponentDao = (DomainObjectDao) AggreateDaoFactory.tryGetDao(oneToManyEntityClass);
