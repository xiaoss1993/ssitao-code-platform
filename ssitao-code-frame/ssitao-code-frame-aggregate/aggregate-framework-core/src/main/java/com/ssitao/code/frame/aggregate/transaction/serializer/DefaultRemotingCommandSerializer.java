package com.ssitao.code.frame.aggregate.transaction.serializer;

import com.ssitao.code.frame.aggregate.remoting.exception.RemotingCommandException;
...
public class DefaultRemotingCommandSerializer implements RemotingCommandSerializer {
    ...
    public RemotingCommand deserialize(byte[] bytes) {
        ...
        if (remarkLength > 0) {
            if (remarkLength > bytes.length) {
                throw new RemotingCommandException("Agg Remoting protocol decoding failed, remark length: " + remarkLength + ", but total length: " + bytes.length);
            }
        ...
        if (bodyLength > 0) {
            if (bodyLength > bytes.length) {
                throw new RemotingCommandException("Agg Remoting protocol decoding failed, body length: " + bodyLength + ", but total length: " + bytes.length);
            }
