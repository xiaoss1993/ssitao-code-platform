...
import com.ssitao.code.frame.aggregate.transaction.serializer.DomainStoreMapSerializer;
import com.ssitao.code.frame.aggregate.transaction.serializer.TransactionStoreMapSerializer;
...
public abstract class AbstractRedisTransactionStorage extends AbstractKVTransactionStorage<Jedis> {
    ...
    protected Long createByScriptCommand(RedisCommands commands, TransactionStore transactionStore) {
        ...
        for (Map.Entry<byte[], byte[]> entry : TransactionStoreMapSerializer.serialize(transactionStore, true)
                .entrySet()) {
    ...
    protected Long updateByScriptCommand(RedisCommands commands, TransactionStore transactionStore) {
        ...
        for (Map.Entry<byte[], byte[]> entry : TransactionStoreMapSerializer.serialize(transactionStore,false)
                .entrySet()) {
            params.add(entry.getKey());
            params.add(entry.getValue());
        }
        ...
        Object result = commands.eval(String.format(
                        ...
                        transactionStore.getVersion() - 1).getBytes(StandardCharsets.UTF_8),
    ...
    @Override
    protected Page<byte[]> findKeysFromOneShard(String domain, Jedis shard, String currentCursor, int maxFindCount, boolean isMarkDeleted) {
    ...
    public void updateDomain(DomainStore domainStore) {
        ...
        try (RedisCommands commands = getRedisCommands(domainStoreRedisKey)) {
            ...
            Object result = commands.eval(String.format(
                            "if redis.call('hget',KEYS[1],'VERSION') == '%s' then redis.call('hmset', KEYS[1], unpack(ARGV)); return 1; end; return 0;",
                            domainStore.getVersion() - 1).getBytes(StandardCharsets.UTF_8),
                    Arrays.asList(domainStoreRedisKey),
                    params);
