package com.ssitao.code.frame.aggregate.dashboard.utils;

import io.jsonwebtoken.Claims;
import io.jsonwebtoken.Jwts;
import io.jsonwebtoken.SignatureAlgorithm;

import com.ssitao.code.frame.aggregate.dashboard.model.SystemUser;
...
public class JwtUtil {
    ...
    private static final String JWT_SECRET = "aggregate-framework";
    ...
    private static Claims getClaimsFromToken(String token) {
        Claims claims;
        try {
            claims = Jwts.parser().setSigningKey(JWT_SECRET).build().parseSignedClaims(token).getPayload();
        } catch (Exception e) {
            claims = null;
        }
        return claims;
