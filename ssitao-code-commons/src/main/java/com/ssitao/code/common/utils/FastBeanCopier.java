...
public final class FastBeanCopier {
    private static final Map<CacheKey, Copier> CACHE = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    public static Copier createCopier(Class<?> source, Class<?> target) {
        ...
        String method = "public void copy(Object s, Object t, java.util.Set ignore, " +
                "com.ssitao.code.common.utils.Converter converter){\n" +
                "try{\n\t" +
                ...
                "\n}";
        try {
            return DynamicProxy.create(Copier.class)
                    .addMethod(method)
                    .newInstance();
        } catch (Exception e) {
    ...
    static abstract class ClassProperty {
        ...
        public BiFunction<Class<?>, Class<?>, String> createGetterFunction() {
            ...
            return (targetBeanType, targetType) -> {
                ...
                String generic = "com.ssitao.code.common.utils.FastBeanCopier.EMPTY_CLASS_ARRAY";
                Field field = ReflectionUtils.findField(targetBeanType, name);
                boolean hasGeneric = false;
                if (field != null) {
                    String[] arr = Arrays.stream(org.springframework.core.ResolvableType.forField(field)
                                    .getGenerics())
                            .map(org.springframework.core.ResolvableType::getRawClass)
